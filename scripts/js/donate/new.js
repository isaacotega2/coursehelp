...
//alert ( JSON.stringify(response) );
			...
			for(var i = 0; i < response["courses"].length; i++) {
				...
				var option = (course["acceptingDonation"] ? ('<input name="course" type="radio" id="chk' + course["courseId"] + '" value="' + course["courseId"] + '">') : '') + '<label for="chk' + course["courseId"] + '">' + course["name"] + (course["acceptingDonation"] ? ('') : ' (Unavailable)') + '</label>';
				
				$("#newDonationForm").find("#coursesHolder #content").append(option + ' <br><br>');
