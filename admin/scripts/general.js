	...
	$.ajax({
		...
		//	alert ( JSON.stringify(response[0]) );
			...
			//	alert();
				...
				var row = $('<tr id="' + course["courseId"] + '" class="courseRow"></tr>').html('<td>' + index + '</td> <td>' + course["name"] + '</td> <td>' + course["department"]["name"] + '</td> <td>' + course["institution"]["name"] + '</td> <td>' + course["level"] + '</td> <td>' + course["handouts"]["number"] + '</td> <td>' + course["datePosted"] + " | " + course["timePosted"] + '</td> <td> <div id="loader"></div>  <input type="checkbox" id="chkAcceptingDonation" ' + (course["acceptingDonation"] ? "checked" : "") + '> </td> </tr>');
			...
			$("#tblCourses tbody [type=checkbox]").change(function () {
				...
				$.ajax({
					...
					dataType: "JSON",
					data: {
						courseId: $(this).parents("tr").attr("id"),
						request: "toggleAcceptDonation"
					...
					//	alert ( JSON.stringify(response) );
						...
						$(checkbox).siblings("#loader").hide();
				
						$(checkbox).attr("checked", response["acceptingDonation"].toString());
