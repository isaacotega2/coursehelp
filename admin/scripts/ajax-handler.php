<?php
	...
	if($request == "toggleAcceptDonation") {
		
		$courseId = $_POST["courseId"];
		
		$courseDetails = courseDetails($courseId);
		
		if($courseDetails["acceptingDonation"]) {
		
			$sql = "UPDATE courses SET accepting_donation = 'false' WHERE course_id = '$courseId' ";
			
		}
		
		else {
		
			$sql = "UPDATE courses SET accepting_donation = 'true' WHERE course_id = '$courseId' ";
			
		}
			
			if(mysqli_query($conn, $sql)) {
		
				echo json_encode(array("status" => "success", "acceptingDonation" => courseDetails($courseId)["acceptingDonation"] ));
				
			}
		
	}
	
	if($request == "downloadDonatedHandouts") {
		
		function fileContent($path, $pageNumber) {
		
			global $handoutDetails, $page;
			
			$filename = ($handoutDetails["handoutId"] . "_" . $pageNumber . ".jpg");
			
			$defaultImage = ($page["rootPath"] . "images/empty-image.jpg");
		
			$result = file_get_contents(file_exists($path . $filename) ? ($path . $filename) : $defaultImage);
		...
		$accountDetals = accountDetails($handoutDetails["donatorUsercode"]);
		
		$courseDetails = courseDetails($handoutDetails["courseId"]);
		
		$departmentDetails = departmentDetails($handoutDetails["departmentId"]);
		
		$institutionDetails = institutionDetails($handoutDetails["institutionId"]);
		
		$readmeText = ('Posted by: ' . $accountDetals["nickname"] . '
		
Name: ' . $handoutDetails["name"] . '
...
Level: ' . $handoutDetails["level"] . '

Date: ' . $handoutDetails["datePosted"] . " | " . $handoutDetails["timePosted"]);
		
		
		$zip = new ZipArchive();

		$filename = ("../archives/" . $handoutDetails["name"] . "_by_" . $accountDetals["nickname"] . ".zip");
		...
		for($i = 0; $i < $handoutDetails["pagesNumber"]; $i++) {
			...
			$path = ($page["rootPath"] . "images/handouts/");
			
			$zip->addFromString(("Page " . $index . ".jpg"), fileContent($path, $index));
	...
	if($request == "saveHandout") {
		...
		foreach($courseDetails["handouts"]["ids"] as $eachId) {
			...
			if($handoutId == $eachId) {
				...
				die(json_encode(array("status" => "error", "cause" => "handoutExists")));
	...
	if($request == "rejectHandout") {
		...
		$message = mysqli_real_escape_string($conn, $_POST["message"]);
			...
			$sql = "UPDATE donated_handouts SET status = 'rejected', rejection_message = '$message' WHERE handout_id = '$handoutId' ";
